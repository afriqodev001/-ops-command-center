"""
Harness browser session management — sidebar widget + connect/disconnect.
Same pattern as SPLOC, Splunk, ServiceNow, Tachyon, and Copilot.
"""
...
_INTEGRATION = 'harness'


def _harness_base():
    from django.conf import settings as s
    return getattr(s, 'HARNESS_BASE', 'https://your-org.harness.io').rstrip('/')
...
def _build_session_context():
    ...
    if not session:
        ...
        if all_:
            _, session = next(iter(all_.items()))

    if not session:
        return {
            'harness_status': 'none',
            'harness_status_label': 'No session',
    ...
    return {
        ...
        'harness_status_label': label,
        'harness_session': session,
        'harness_port': port,
        'harness_last_used_ago': last_used_ago,
    }
...
def session_widget(request):
    ...
    return render(request, 'harness/partials/session_widget.html', ctx)
